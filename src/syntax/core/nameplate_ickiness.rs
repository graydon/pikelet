...
impl TermLam {
    ...
    pub fn unbind(mut self) -> (Named<Name, Option<RcTerm>>, RcTerm) {
        ...
        (
            self.unsafe_param,
            self.unsafe_body
                .open(&Term::Var(Span::start(), Var::Free(fv)).into()), // FIXME: Span::start()
        )
    }
}
...
pub fn unbind2(
    ...
    (
        lam.unsafe_param,
        lam.unsafe_body
            .open(&Term::Var(Span::start(), Var::Free(fv.clone())).into()), // FIXME: Span::start()
        pi.unsafe_param,
...
impl RcTerm {
    ...
    pub fn close_at(&mut self, level: Debruijn, name: &Name) {
        *self = match *Rc::make_mut(&mut self.inner) {
            ...
            Term::Var(span, Var::Free(ref n)) if n == name => {
                Term::Var(span, Var::Bound(Named::new(n.clone(), level))).into()
            },
    ...
    pub fn open_at(&self, level: Debruijn, x: &RcTerm) -> RcTerm {
        match *self.inner {
            ...
            Term::Lam(start, ref lam) => {
                ...
                Term::Lam(start, lam).into()
            },
            Term::Pi(start, ref pi) => {
